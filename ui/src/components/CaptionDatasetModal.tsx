...
export const CaptionDatasetModal: React.FC = () => {
  ...
  const saveJob = async () => {
    ...
    if (!modalInfo?.datasetPath) {
      alert(translate('Dataset path is missing. Please try again.'));
      return;
    }
    ...
    apiClient
      ...
      .catch(error => {
        if (error.response?.status === 409) {
          alert(translate('A caption job for this dataset already exists. Please check your jobs list.'));
        } else {
          alert(translate('Failed to save job. Please try again.'));
        }
  ...
  return (
    <Modal
      isOpen={open}
      onClose={handleClose}
      title={translate('Caption Dataset')}
      size={activeTab === 'advanced' ? 'xl' : 'lg'}
    >
