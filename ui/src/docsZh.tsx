...
const docs: { [key: string]: ConfigDoc } = {
  ...
  'config.process[0].trigger_word': {
    ...
    description: (
      <>
        ...
        <br />
        <br />
        触发词不会自动加到测试提示词中，请手动添加或同样使用 <code>{'[trigger]'}</code> 作为占位符。
      </>
    ),
  },
  'config.process[0].model.name_or_path': {
    ...
    description: (
      <>
        HuggingFace 上 diffusers 仓库名，或本地基础模型的文件夹路径。多数模型需要使用 diffusers
        格式的文件夹；部分模型（如 SDXL、SD1）可填入整合的 safetensors 检查点路径。
      </>
    ),
  },
  'config.process[0].model.arch': {
    title: '模型架构',
    description: <>选择训练所用的基础模型架构，会影响可配置项与训练流程。例如图像、视频、编辑模型的架构不同。</>,
  },
  'model.low_vram': {
    title: '低显存',
    description: <>启用后以更保守的显存占用进行训练，适合显存较小的显卡。可能降低训练速度但提高稳定性。</>,
  },
  ...
  'model.layer_offloading_text_encoder_percent': {
    title: '文本编码器卸载百分比',
    description: <>设置将文本编码器权重卸载到 CPU 内存的比例（0–100%）。用于进一步降低显存占用。</>,
  },
  'config.process[0].network.type': {
    title: '目标类型',
    description: <>训练目标网络类型，常见为 LoRA 或 LoKr。不同类型会影响参数含义与导出格式。</>,
  },
  'config.process[0].network.lokr_factor': {
    title: 'LoKr 因子',
    description: <>LoKr 的因子设置，影响参数分解与容量。-1 表示自动选择。</>,
  },
  'config.process[0].network.linear': {
    title: '线性秩',
    description: <>LoRA 的线性层秩（rank），数值越大容量越高但显存与训练难度也增加。</>,
  },
  'config.process[0].network.conv': {
    title: '卷积秩',
    description: <>卷积分支的秩，控制卷积通道的低秩近似容量。可选项，按需调整。</>,
  },
  'train.batch_size': {
    title: '批大小',
    description: <>每步训练的样本数量。受显存影响，过大可能 OOM，过小训练不稳定。</>,
  },
  'train.gradient_accumulation': {
    title: '梯度累计',
    description: <>累计多次小批次的梯度再进行一次优化，相当于放大有效批大小以节省显存。</>,
  },
  'train.steps': {
    title: '总步数',
    description: <>总训练迭代步数。步数越多通常效果更好，但训练时间更长。</>,
  },
  'train.optimizer': {
    title: '优化器',
    description: <>选择参数更新算法，如 AdamW8Bit 或 Adafactor。不同优化器对显存与稳定性影响不同。</>,
  },
  'train.lr': {
    title: '学习率',
    description: <>控制参数更新的步幅。过大易发散，过小收敛慢。建议从 1e-4 等常用值起调。</>,
  },
  'train.optimizer_params.weight_decay': {
    title: '权重衰减',
    description: <>L2 正则项系数，抑制过拟合并提升泛化能力。通常与优化器搭配调整。</>,
  },
  'train.timestep_type': {
    title: '时间步类型',
    description: <>噪声时间步分布类型（Sigmoid/Linear/Shift/Weighted），影响训练采样策略与学习重点。</>,
  },
  'train.content_or_style': {
    title: '时间步偏向',
    description: <>训练偏向形体、结构（高噪声）或细节、纹理（低噪声），Balanced 表示均衡。</>,
  },
  'train.loss_type': {
    title: '损失类型',
    description: <>选择损失函数（MSE/MAE/Wavelet/Stepped Recovery），影响训练优化目标与收敛特性。</>,
  },
  ...
  'datasets.num_frames': {
    ...
    description: (
      <>
        ...
        <br />
        <br />
        建议在训练前将视频剪裁到合适长度。以 Wan 为例，默认 16fps，81 帧约等于 5 秒视频， 因此将视频统一到约 5
        秒更利于训练稳定。
        <br />
        <br />
        示例：若设为 81，且数据集中两个视频分别为 2 秒与 90 秒，都会被均匀抽取为 81 帧， 因此 2 秒视频看起来更慢，90
        秒视频看起来更快。
      </>
    ),
  },
  ...
  'model.layer_offloading': {
    ...
    description: (
      <>
        ...
        <br />
        <br />
        层级卸载会使用 CPU 内存来承载模型的大部分权重，而不是使用 GPU
        显存。这使得在较小显存的显卡上也能训练更大的模型（前提是拥有足够的 CPU 内存）。 相比纯 GPU
        显存训练，这种方式速度更慢，但 CPU 内存更便宜且可升级。仍然需要一定的 GPU 显存来保存优化器状态与 LoRA
        权重，通常仍建议使用较大显存的显卡。
        <br />
        <br />
        你可以选择需要卸载的层所占的百分比。一般来说，为了性能更好，建议尽量少卸载（接近
        0%）；如果内存不足，可以适当提高卸载比例。
      </>
    ),
  },
  ...
  'train.diff_output_preservation': {
    ...
    description: (
      <>
        ...
        训练过程中会进行一次“先验预测”：禁用 LoRA，将提示词中的触发词替换为类别词（例如“photo of Alice”→“photo of
        woman”）。 每一步除了正常训练外，还会基于该先验预测与类别提示执行一次额外训练，以帮助 LoRA 保持对类别的认知。
  ...
  'train.do_differential_guidance': {
    ...
    description: (
      <>
        ...
        <br />
        <img
          src="/imgs/diff_guidance_cn_clean.svg"
          alt="差分引导原理图"
